CliOptions = Ext.extend(Object, {
    ...
    add: function(opt) {
        ...
        if (!opts[name]) {
            opt = opts[name] = Ext.take(opt, ['name', 'alias', 'description', 'attrs', 'defaultValue', 'validator']);
        ...
        var me = this,
            ...
            isValid = function() {
                ...
                if (opt.toggled) {
                    ...
                    if (opt.validator && !opt.validator.call(opt, opt.value)) {
                        return false;
                    }
                }
    ...
    parse: function(args) {
        ...
        for(; i < len, arg = args[i]; i++) {
            ...
            if ((match = arg.match(dashSwitchRe)) || (match = arg.match(dblDashSwitchRe))) {
    ...
    printUsage: function() {
        var opts = this.options,
            ...
            descs = [],
            i = 0,
            opt, len, name, valueOptional, line, words;
        ...
        for (len = keys.length; i < len; i++) {
            ...
            name = '--' + opt.name + (takesValue ? '[=]' + (valueOptional ? '[value]' : 'value') : '');
            if (opt.alias) {
